...
数据文件（bin/txt/lua）按目标自动分离到子目录：
- `-t client` → `output/bin/client/`, `output/txt/client/`, `output/lua/client/`
- `-t server` → `output/bin/server/`, `output/txt/server/`, `output/lua/server/`
- `-t all` → `output/bin/all/`, `output/txt/all/`, `output/lua/all/`
...
# 导出客户端数据（自动输出到 output/bin/client/）
python main.py -i <xlsx目录> -o output/bin -f bin -t client

# 导出服务器数据（自动输出到 output/txt/server/）
python main.py -i <xlsx目录> -o output/txt -f txt -t server

# 导出客户端 Lua（自动输出到 output/lua/client/）
python main.py -i <xlsx目录> -o output/lua -f lua -t client

# 生成 C# + Go 代码
python main.py -i <xlsx目录> -f code --csharp-out <C#目录> --go-out <Go目录>

# 指定配置文件
python main.py -i <xlsx目录> -o output/bin -f bin -t client -c config.json

# 详细日志
python main.py -i <xlsx目录> -o output/txt -f txt -t client -v
```
